        ...
        for AS_NAME in ALL_ASs:
            ...
            for name, ip in AS.networks_config[subnet].items():
                if new_AS_NAME in name and '@' in name:
                    ...
                    if internal_name in nodes['Borderrouter'] or internal_name in nodes['Internal-Router']:
                        continue
...
def test_SCION_ping(ASes, AS):
    ...
    for host in net.hosts:
        ...
        for destination in all_destinations: 
            ...
            command = f"cd {AS.SCION_PATH} ; ./bin/scion ping --sciond {ip_sciond}:30255 --dispatcher {dispatcher_socket} {destination} -c 1 --timeout 3s"
...
def test_SCION_bwtest_client_server(ASes, networks, AS):
    ...
    command = f'cd {SCION_APPS} ; export SCION_DAEMON_ADDRESS="{server_ip_sciond}:30255" ; export SCION_DISPATCHER_SOCKET={sever_dispatcher_socket} ; ./bin/scion-bwtestserver --listen=:{PORT} &'
    lg.debug(command)
    ...
    for ASstr in ASes.keys():
        ...
        command = f'cd {SCION_APPS} ; export SCION_DAEMON_ADDRESS="{client_daemon}:30255" ; export SCION_DISPATCHER_SOCKET={client_dispatcher_socket} ; ./bin/scion-bwtestclient -s {server_addr}:{PORT} -sc {SC_string} -cs {CS_string}'
        lg.debug(command)
